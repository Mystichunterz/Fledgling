...
| **Fal** | Pixel-art sprite sheets and walk-cycle animations (build-time) | Hosts a pixel-art-specific model that holds 32×32 cleanly where SDXL gives sub-pixel mush. A manifest-driven `scripts/gen-sprites.ts` runs each NPC × pose × held-object combination with a pinned per-character seed, so Naro stays Naro across her walk cycle — facial detail is a few pixels wide and even small drift makes the character read as a different person. Animations are discrete keyframes (*foot forward, planted, back*) with shared seeds — we tried CogVideoX-style continuous output first and got beautiful but inconsistent drift between frames. A Sharp pipeline quantises to a 16-colour ramp and packs into one texture atlas with type-safe keys regenerated into `src/assets/keys.ts`. Build-time because the title screen has a two-second budget and even the fastest pixel-art model is slower than that — we eat the cost once so the player doesn't. |
...
| *GPT-5.5 Realtime + Image v2* (planned, unshipped) | Per-run noun sprites | Fires during the loading screen to draw fresh sprites for the run's invented nouns — *fish* in this run gets a fish drawn to match the village's vibe. Pipeline scoped, hours ran out. Sprites currently come from Fal at build-time. |
...
- **Real-time noun-image generation.** The plan was to fire GPT-5.5 Realtime + Image v2 during the loading screen to generate sprites for the run's invented nouns — so the word for *fish* in this run gets a fish sprite drawn fresh, matching the village's vibe. Pipeline was scoped and the loading-screen budget was there. We ran out of hours. Sprites currently come from Fal at build-time.
...
- **Fal** for pixel-art sprite sheets and walk-cycle animation frames (build-time)
- *GPT-5.5 Realtime + Image v2 for per-run noun sprites (planned, unshipped)*
