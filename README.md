...
```
Run #1, Pemi the baker:    "tova-n  ki-ra  ne-ko  sov."
Run #2, Pemi the baker:    "jino-fas  vetaru  no-pa."
Run #3, Pemi the baker:    "se kuru-le  pana-mi."
```
...
The language is invented per run. You can't have studied it. You can't look it up. The 50-word lexicon, the suffix rules, the SOV word order, the phonotactics get generated by GPT-5.5 from a seed when the page loads, then voiced by ElevenLabs in voices we hand-tuned per villager. The game is the same every run. The language never is.

You don't need a linguistics background. There's no tutorial. A 17-second prologue tells you you've crashed, that nobody speaks your language, that you can hover any word to write what you think it means, and that your guesses are the only translation you're going to get. Then a child runs across the sand to greet you and the village starts teaching you itself.
...
Pemi walks over. She says: **"tova-n ki-ra ne-ko sov."**
...
That's the loop. Fifteen exchanges later you're starting to feel the shape of it. By the time you climb the lighthouse, you can hold a real conversation with Hala — out loud, in a language you couldn't have heard before this morning.

---

## The AI stack

Six AI services. Each one earns its slot.

| Service | Role | Why this one |
|---|---|---|
| GPT-5.5 | Procedural language generator | Generates a full `LanguageSpec` per seed: phonology, 50-word lexicon, morphology rules, syntax. Same seed, same language — share a tongue by sharing a seed. |
...
- **Per-run NPC portrait variation.** Same idea, same pipeline, same reason it's still on the list.
...
These aren't blockers — the game ships and is fun without them — but they're the obvious next move. Three of the six AI services are already wired for it.
