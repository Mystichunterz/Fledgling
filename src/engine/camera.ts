...
export interface SnappedFollowOptions {
  ...
  viewportW?: number;  // game internal width  (320 default)
  viewportH?: number;  // game internal height (180 default)
}
...
export function snappedFollow(
  ...
  const viewportW = options.viewportW ?? 320;
  const viewportH = options.viewportH ?? 180;

  const cam = scene.cameras.main as Phaser.Cameras.Scene2D.Camera & SnappedFollowState;
  cam.stopFollow();

  cam.__snapTargetX = target.x - viewportW / 2;
  cam.__snapTargetY = target.y - viewportH / 2;
  cam.setScroll(Math.round(cam.__snapTargetX), Math.round(cam.__snapTargetY));

  const onUpdate = () => {
    if (!target.active) return;
    const desiredX = target.x - viewportW / 2;
    const desiredY = target.y - viewportH / 2;
